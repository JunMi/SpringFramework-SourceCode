...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//将资源文件resource编码处理，再继续调用loadBeanDefinitions
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			//从encodedResource总取出已经封装过得资源
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				//inputSource全路径是org.xml.sax.InputSource
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			//得到document文档，具体怎么得到的，进去方法里面看一下
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		/**
		 * 下面的documentLoader通过new DefaultDocumentLoader()得到,
		 * DefaultDocumentLoader通过工厂类DocumentBuilderFactory来创建DocumentBuilder,
		 * 所以真正的实现在createDocumentBuilder方法中,体现了面向对象的单一职责原则！！！
		 * 
		 * 获取document干了两件事，分别是getEntityResolver和getValidationModeForResource，下面进去看一下具体干了啥！
		 */
